...
import type { DesktopSlideLayout } from "../layouts";
    ...
    ({ layout, getItemById }: DesktopSlideProps) => {
        ...
        if (layout.fullWidth) {
            layout.fullWidth.forEach(id => {
                ...
                if (item) {
                    renderedItems.push(
                        <div key={`full-${id}`} className="w-full">
                            {renderCard(item)}
                        </div>
                    );
                }
                ...
                row.forEach(id => {
                    ...
                    if (item) {
                        currentRowItems.push(
                            <div key={`row-${rowIdx}-${id}`} className="flex-1">
                                {renderCard(item)}
                            </div>
                        );
                    } else {
        ...
        if (layout.fullHeightLeft && layout.fullHeightLeft.length > 0) {
            const leftColumn = (
                <div key="left-column" className="lg:flex-shrink-0">
                    {layout.fullHeightLeft.map(id => {
                        const item = getItemById(id);
                        if (item) {
                            return (
                                <div key={`full-left-${id}`}>
                                    {renderCard(item)}
                                </div>
                            );
                        }
            ...
            const rightColumn = (
                <div key="right-column" className="flex flex-col gap-5 flex-1">
                    {renderedItems}
                </div>
            );
        ...
        if (layout.fullWidthRight && layout.fullWidthRight.length > 0) {
            const leftColumn = (
                <div key="left-column" className="flex flex-col gap-5 flex-1">
                    {renderedItems}
                </div>
            );

            const rightColumn = (
                <div key="right-column" className="lg:flex-shrink-0">
                    {layout.fullWidthRight.map(id => {
                        const item = getItemById(id);
                        if (item) {
                            return (
                                <div key={`full-right-${id}`}>
                                    {renderCard(item)}
                                </div>
                            );
                        }
        ...
        return <div className="flex flex-col gap-5">{renderedItems}</div>;
    }
);
